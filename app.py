...
def feedback():
    if not session.get('username'):
        ...
        if request.method=='POST':
            ...
            feed = Feed(feedback=com, rate=exp,user_id=uid )
            db.session.add(feed)
            db.session.commit()

            

            flash('Your Feedback Successfully submit !','success') 
...
def login():
    ...
    if request.method == "POST":
        ...
        if user :
            ...
            if not session['username']=='ahmed_aqeel97@yahoo.com':
...
@app.route("/delete/feedback/<id>", methods=['GET', 'POST'])
def delete_ord(id):
    if not session.get('admin') :
        session.clear()
        flash('Unauthorized access login correctly! ','danger')
